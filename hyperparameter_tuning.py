...
def vectorize_data(params):
    ...
    ## Save the selective smiles
        data_list = pd.DataFrame(smiles,columns=["smiles"])
        data_list.to_csv("./models/MP_properties/selective_smiles.csv",index=False)
...
def main_no_prop(params):
    ...
    #csv_clb = CSVLogger("history_data_size_test/"+params['data_file_name']+".csv", append=False)
    csv_clb = CSVLogger("history_without_kl_wheight_schedule/test_with.csv", append=False)
    #tbCallBack = TensorBoard(log_dir='tensorboard_log_data_size_test/'+params['data_file_name'],write_grads=True,write_images=True)
    tbCallBack = TensorBoard(log_dir='tensorboard_log_without_kl_wheight_schedule/test_with',write_grads=True,write_images=True)
    ...
    #         print(np.float32(K.eval(lr_t)))
    #         return np.float32(K.eval(lr_t))

    #     def on_epoch_end(self, episode, logs = {}):
    #         logs.update({"lr": self.lr_getter()})   
    #         super(LRTensorBoard, self).on_epoch_end(episode, logs)
    # reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.2,
    #                           patience=5, min_lr=0.001)
    
    
    #tbCallBack = TensorBoard(log_dir='test_continue',write_grads=True,write_images=True)
    
    callbacks = [ vae_anneal_callback,tbCallBack,csv_clb]
...
if __name__ == "__main__":
    ...
    params["epochs"] = 200
    params["hidden_dim"] = 196
    params["data_file"] = "data_size_test/60W.csv"
