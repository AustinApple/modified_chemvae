...
def vectorize_data(params):
    ...
    if (params['do_prop_pred_only'] and ('data_file' in params)) or (params['do_prop_pred'] and ('data_file' in params)):
    ...
    if params['do_prop_pred'] or params['do_prop_pred_only']:
...
def load_models(params):
    ...
    if params['do_prop_pred']:
        ...
        reg_prop_pred = property_predictor(z_mean)
        reg_prop_pred = Lambda(identity, name='reg_prop_pred')(reg_prop_pred)
        PP_model = Model(x_in,reg_prop_pred)
        return PP_model, encoder, decoder, property_predictor, kl_loss_var
...
def only_property_run(params):
    start_time = time.time()
    print("Now start to train property predictor...............")
    # load data
    X_train, X_test, Y_train, Y_test = vectorize_data(params)
    
    # load full models:
    PP_model, encoder, decoder, property_predictor, kl_loss_var = load_models(params)
       # compile models
    if params['optim'] == 'adam':
        optim = Adam(lr=params['lr'], beta_1=params['momentum'])
    elif params['optim'] == 'rmsprop':
    ...
    # model_losses['reg_prop_pred'] = params['reg_prop_pred_loss']
    model_losses['reg_prop_pred'] = root_mean_squared_error
    model_loss_weights['reg_prop_pred'] = params['prop_pred_loss_weight']
    
    csv_clb = CSVLogger("predict_training_log.csv", append=False)
    Checkpoint = ModelCheckpoint("Model_70W_MP_property/weights.{epoch:02d}-{val_loss:.4f}.hdf5", monitor='val_loss', 
                                  verbose=0, save_best_only=True, save_weights_only=False, mode='auto',
                                  period=1)
    tbCallBack = TensorBoard(log_dir='tensorboard_log_70W_MP_property',write_grads=True,write_images=True)

    # control verbose output
    keras_verbose = params['verbose_print']
    PP_model.compile(loss=model_losses,
               loss_weights=model_loss_weights,
               optimizer=optim)
    ...
    PP_model.fit(X_train, model_train_targets,
                         ...
                         epochs=params['epochs'],
                         initial_epoch=params['prev_epochs'],
                         callbacks=[csv_clb,Checkpoint,tbCallBack],
                         verbose=1,
         validation_data=[X_test, model_test_targets]
     )
